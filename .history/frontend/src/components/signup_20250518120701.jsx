...
const SignUp = () => {
 ...
 axios.post("http://localhost:3000/api/users/signup",userInfo).then((res)=>{
  console.log(res.data);
  if (res.data.success) {
    setuser(res.data.user);
    setError(null);
    localStorage.setItem("userdata", JSON.stringifyres.data);
          ...
          >
            ...
            {error && (
              <p
               ...
               className="text-red-500 text-center mt-4">{error}</p>
            )}
          </form>
        </div>
      </div>
