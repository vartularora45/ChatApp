  ...
  const handleSubmit = async(e) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:3000/api/users/signup", // Changed from login to signup
        userInfo,
        {
          headers: {
      ...
      if (res.data.success || res.data._id || res.data.status === "success") {
        ...
        // Store user info in localStorage & context
        localStorage.setItem("user", JSON.stringify(res.data));
        setUser(res.data);

        // Redirect after a short delay
        setTimeout(() => {
          navigate("/home"); // Or wherever you want to redirect after signup
        }, 1500);
      }
    } catch (err) {
